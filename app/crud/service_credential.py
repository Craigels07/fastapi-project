...
from app.schemas.service_credential import ServiceCredentialCreate, ServiceCredentialUpdate
from app.utils.encryption import encrypt_data, decrypt_data


def create_service_credential(db: Session, credential: ServiceCredentialCreate) -> ServiceCredential:
    """
...
def get_service_credential(db: Session, credential_id: UUID) -> Optional[ServiceCredential]:
    """
    Get a service credential by ID
    """
    return db.query(ServiceCredential).filter(ServiceCredential.id == credential_id).first()
...
def get_service_credentials_by_org(
    ...
    """
    query = db.query(ServiceCredential).filter(ServiceCredential.organization_id == organization_id)
...
def get_decrypted_credentials(db: Session, credential_id: UUID) -> Optional[Dict[str, Any]]:
    """
...
def get_organization_service_credentials(db: Session, organization_id: UUID) -> Dict[str, Dict[str, Any]]:
    """
    ...
    for cred in db_credentials:
        ...
        service_type = cred.service_type.value if isinstance(cred.service_type, ServiceTypeEnum) else cred.service_type
